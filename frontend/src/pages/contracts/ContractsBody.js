...
const ContractsBody = ReactRouterDOM.withRouter(function () {
  ...
  const [dataSourceCustomerId, setDataSourceCustomerId] = React.useState([]);

  React.useEffect(() => {
    fetchDataSourceCustomerId();
  }, []);
  ...
  const fetchDataSourceCustomerId = async function () {
    ...
    setDataSourceCustomerId(records.map((record) => [record.customerId, record.name]));
  }
            ...
            filters={[
              {
                ...
                variant: 'option',
                dataSource: dataSourceCustomerId
              },
              {
                label: words.status,
                variant: 'option',
