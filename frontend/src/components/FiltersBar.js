...
const FiltersBar = function ({ sessionStorageKey, filters, filtersValues, setFiltersValues }) {
    ...
    switch (filter.variant) {
      ...
      case 'option': {
        ...
        for (const item of filterValue) {
          const dataSourceItem = filter.dataSource.find((dataSourceItem) => dataSourceItem[0] === item);
          if (dataSourceItem) {
  ...
  return (
    <div>
      <div ref={filterBarRef} className="wit-filters-bar d-flex align-items-center">
