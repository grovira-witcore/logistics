  ...
  return (
    <div>
      <div className={'form-group' + (inline ? '-inline' : '')} data-invalid={errors.length > 0 ? '1' : null}>
        ...
        <input ref={refInput} className={'form-control' + (inline ? '-inline' : '') + (validated && errors.length > 0 ? ' is-invalid' : '')} type="text" placeholder={placeholder} value={friendlyName !== null && friendlyName !== undefined ? friendlyName : ''} onClick={handleClick} onKeyDown={handleKeyDown} onFocus={(e) => e.target.select()} onBlur={handleOnBlur} />
