...
module.exports = async function (knex, req, res) {
  try {
    const { code, bookingId, pallets, grossLb, transporterId, baseCost } = req.body;
    ...
    res.status(201).send({ cargoId });

  } catch (error) {
    console.error(error);
    res.status(500).send({ error: 'An error occurred while creating the cargo.' });
  }
}
